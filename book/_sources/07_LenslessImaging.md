...
with the intensities $\mathbf{s}$ emerging from every sampled scene point, the measurement matrix $\mathbf{H}$ and the resulting measurements $\mathbf{g}$.

+++ {"slideshow": {"slide_type": "fragment"}}

This model can be interpreted as if every column $\mathbf{H}[i]$ of $\mathbf{H}$ stores the sensor image that is associated with the corresponding scene point (i.e., element $s_i$ of $\mathbf{s}$). The value of $s_i$ (i.e., the reflected intensity of that scene point) then scales $\mathbf{H}[i]$ before the images corresponding to all captured scene points are additively superposed and yield the measurement $\mathbf{g}$.
...
The general linear model can be approximated as a linear shift-invariant system via a convolution of the scene intensities $s(m,n)$ with the PSF $h(m,n)$, i.e.,
...
$\begin{align} 
   \mathbf{g} = \Phi_\mathrm{L}\Phi_\mathrm{R}\transp \mathbf{s}
\end{align}$
...
$\begin{align} 
   \mathbf{g} = \sum\limits_z \mathbf{H}_z \mathbf{s}_z \,.
\end{align}$
...
$\begin{align} 
   \mathbf{g} = \sum\limits_z \Phi_{\mathrm{L},z}\Phi_{\mathrm{R},z}\transp \mathbf{s}_z \,.
\end{align}$
...
In order to perform the reconstruction of the sought scene intensities $\mathbf{s}$ for an actual lensless imaging system based on an observation $\mathbf{g}$, 

* the system's forward model has to be correctly formulated and 
* the model has to be calibrated,

so that the forward model can be evaluated for an estimate $\hat{\mathbf{s}}$ of the scene intensities.

+++ {"slideshow": {"slide_type": "fragment"}}

Possible approaches to reconstruct $\mathbf{s}$ are deconvolution methods and convex optimization algorithms as discussed in the previous chapter.
...
where the observation is the result of an additive superposition of all PSFs $\mathbf{H}$ for all possible scene point locations scaled by the corresponding light intensities $\mathbf{s}$. 
...
$\begin{align} 
   g = \mathcal{C}\left\{  \sum\limits_z s_z * h_z  \right\} \,,
\end{align}$

with $s_z$ denoting the intensities of scene points at depth $z$, $h_z$ representing the PSF at depth $z$ and $\mathcal{C}$ denoting a linear operator that crops the image to actual sensor size.
...
For the first approach, we will not use any prior and just try to find an estimate $\hat{\mathbf{s}}$ that optimizes the data fidelity term:

$\begin{align} 
   \hat{\mathbf{s}} &= \argmin{\mathbf{s}} \frac{1}{2} \left\| \underbrace{\mathbf{CH}}_{=:\mathbf{A}}\mathbf{s} - \mathbf{g} \right\|^2_2  \\
   &= \argmin{\mathbf{s}} \underbrace{\frac{1}{2} \left\| \mathbf{A}\mathbf{s} - \mathbf{g} \right\|^2_2}_{=:f(\mathbf{s})} \,,
\end{align}$
...
To perform gradient descent, we have to derive the gradient $\nabla_\mathbf{s} f(\mathbf{s})$ of $f$ with respect to $\mathbf{s}$:

$\begin{align} 
   \nabla_\mathbf{s} f(\mathbf{s}) = \nabla_\mathbf{s} \frac{1}{2} \left\| \mathbf{A}\mathbf{s} - \mathbf{g} \right\|^2_2 = \mathbf{A}\transp (\mathbf{As}-\mathbf{g}) = \mathbf{H}\transp \mathbf{C}\transp \left( \mathbf{CHs} - \mathbf{g}  \right) \,.
\end{align}$
...
As the reconstruction should only have positive pixel values, we replace any negative values of $\hat{\mathbf{s}}$ with 0 during the gradient descent iterations. This is also called *projected gradient descent* since the calculated estimations are projected on the set of images with only positive pixel values.
...
$\begin{align} 
   \hat{\mathbf{s}} = &\argmin{\mathbf{q}} \frac{1}{2} \left\| \mathbf{C} \mathbf{q} - \mathbf{g} \right\| ^2_2 \\ &\text{subject to}\quad \mathbf{q} = \mathbf{Hs} \,.
\end{align}$
...
$\begin{align} 
  \hat{\mathbf{s}} = &\argmin{\mathbf{q}, \mathbf{w}} \frac{1}{2} \left\| \mathbf{C} \mathbf{q} - \mathbf{g} \right\| ^2_2 + \mathrm{pos}(\mathbf{w}) \\ &\text{subject to}\quad \mathbf{q} = \mathbf{Hs},\, \mathbf{w}=\mathbf{s} \,,   
\end{align}$
...
$\begin{align} 
  \hat{\mathbf{s}} = &\argmin{\mathbf{q}, \mathbf{w}, \mathbf{u}} \frac{1}{2} \left\| \mathbf{C} \mathbf{q} - \mathbf{g} \right\| ^2_2 + \mathrm{pos}(\mathbf{w}) + \tau \left\| \mathbf{u} \right\|_1  \\ &\text{subject to}\quad \mathbf{q} = \mathbf{Hs},\, \mathbf{w}=\mathbf{s},\, \mathbf{u} = \Psi \mathbf{s} \,,   
\end{align}$
...
$\begin{align} 
   L(\mathbf{\mathbf{u,q,w,s,\tilde{u},\tilde{q},\tilde{w}}}) = &\frac{1}{2} \left\| \mathbf{C} \mathbf{q} - \mathbf{g} \right\| ^2_2 + \mathrm{pos}(\mathbf{w}) + \tau \left\| \mathbf{u} \right\|_1 \\
    & + \frac{\mu_1}{2} \left\| \mathbf{Hs} - \mathbf{q} \right\|^2_2 + \tilde{\mathbf{q}} \transp \left( \mathbf{Hs-q} \right)  \\
    & + \frac{\mu_2}{2}\left\| \Psi \mathbf{s-u} \right\|^2_2 + \mathbf{\tilde{u}} \transp \left( \Psi \mathbf{s-u} \right) \\
    & + \frac{\mu_3}{2} \left\| \mathbf{s-w} \right\| ^2_2 + \mathbf{\tilde{w}} \transp \left( \mathbf{s-w} \right) \,,
\end{align}$
...
$\begin{align} 
   \mathbf{u}_{k+1} &\leftarrow \mathcal{S}_{\tau / \mu_2} \left( \Psi \mathbf{s}_k +  \frac{1}{\mu_2}\tilde{\mathbf{u}}_k \right) \,, \\
   \mathbf{q}_{k+1} &\leftarrow \left( \mathbf{C}\transp \mathbf{C} + \mu_1 \mathbf{I}  \right) ^{-1} \left( \tilde{\mathbf{q}}_k + \mu_1 \mathbf{Hs}_k + \mathbf{C}\transp \mathbf{g} \right) \,, \\
   \mathbf{w}_{k+1} &\leftarrow  \mathrm{max}\left( \frac{1}{\mu_3}\tilde{\mathbf{w}}_k + \mathbf{s}_k, \mathbf{0} \right) \,, \\
   \mathbf{s}_{k+1} &\leftarrow \left( \mu_1 \mathbf{H}\transp \mathbf{H} + \mu_2 \Psi\transp \Psi + \mu_3 \mathbf{I}\right)^{-1} \cdot  \\
   ...
   \tilde{\mathbf{q}}_{k+1} &\leftarrow \tilde{\mathbf{q}}_k + \mu_1 \left( \mathbf{Hs}_{k+1} - \mathbf{q}_{k+1} \right) \,, \\
   \tilde{\mathbf{u}}_{k+1} &\leftarrow  \tilde{\mathbf{u}}_k + \mu_2 \left( \Psi \mathbf{s}_{k+1} - \mathbf{u}_{k+1} \right)  \,, \\
   \tilde{\mathbf{w}}_{k+1} &\leftarrow \tilde{\mathbf{w}}_k + \mu_3 \left( \mathbf{s}_{k+1} - \mathbf{w}_{k+1} \right) \,,
\end{align}$
